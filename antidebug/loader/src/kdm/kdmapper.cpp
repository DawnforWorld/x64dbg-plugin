...
ULONG64 kdmapper::MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	// Failure path: deliberately do NOT free the kernel image. If the entry
	// ran (or any failure raced its invocation), the mapped image may have
	// spawned a system thread that is executing inside it - FreePool here
	// hands those pages back while they are still being fetched, and the
	// image bugchecks 0x50 at an arbitrary later point (G-M2-1, review P0-1
	// philosophy: a reboot is the recovery, freeing a possibly-live image is
	// never worth it).
	kdmLog(L"[+] Image left resident after failure (leak-by-design)" << std::endl);
	return 0;
}
