...
NTSTATUS intel_driver::Load() {
	// vtdbg: the iqvw64e.sys byte array never enters this repository
	// (Intel-proprietary binary, clause 18a). Generate the real resource
	// header into the build tree with vt/tools/gen_iqvw64e_header.py; the
	// committed stub carries an empty array and we fail fast here.
	if (intel_driver_resource::driver_size == 0) {
		kdmLog(L"[-] iqvw64e.sys resource not generated: run vt/tools/gen_iqvw64e_header.py <iqvw64e.sys> (VT_IQVW64E_PATH, see PROVENANCE.md)" << std::endl);
		return STATUS_UNSUCCESSFUL;
	}
	...
	if (!kdmUtils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), intel_driver_resource::driver_size)) { // vtdbg: driver_size (generated header), stub-safe
	...
	// vtdbg: gated behind g_clear_wd_filter_list, off by default (clause 18l).
	if (g_clear_wd_filter_list && !intel_driver::ClearWdFilterDriverList()) {
...
NTSTATUS intel_driver::Unload() {
	...
	int newFileLen = intel_driver_resource::driver_size + (((long long)rand()*(long long)rand()) % 2000000 + 1000); // vtdbg: driver_size (generated header), stub-safe
...
uint64_t intel_driver::AllocatePool(nt::POOL_TYPE pool_type, uint64_t size) {
	...
	if (!CallKernelFunction(&allocated_pool, kernel_ExAllocatePool, pool_type, size, g_pool_tag)) // vtdbg: identity-injected tag (clause 18j), was hardcoded 'BwtE'
		return 0;
...
bool intel_driver::ClearPiDDBCacheTable() { //PiDDBCacheTable added on LoadDriver
	// vtdbg: stub-safe guard - this routine parses the embedded driver's PE
	// header for its TimeDateStamp; with the committed empty resource stub
	// that read would run off a 1-byte buffer. Callers reach it only after
	// Load()'s driver_size fail-fast, but a direct call must not corrupt.
	if (intel_driver_resource::driver_size == 0) {
		return false;
	}
